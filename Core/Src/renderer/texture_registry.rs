...
impl TextureRegistry {
    ...
    pub fn new(texture_dir: &str) -> Self {
        ...
        match fs::read_dir(texture_dir) {
            Ok(entries) => {
                for entry in entries.flatten() {
                    ...
                    if path.extension().is_some_and(|ext| ext == "png") {
                        if let Some(filename) = path.file_name().and_then(|f| f.to_str()) {
                            ...
                            let texture =
                                textures.entry(base_name.to_string()).or_insert_with(|| {
                                    BlockTexture {
                            ...
                            match suffix {
                                Some("top") => {
                                    texture.top = Some(format!("{}/{}", texture_dir, filename))
                                }
                                Some("bottom") => {
                                    texture.bottom = Some(format!("{}/{}", texture_dir, filename))
                                }
                                Some("side") => {
                                    texture.side = Some(format!("{}/{}", texture_dir, filename))
                                }
                ...
                info!(
                    "Loaded {} unique block textures from directory: {}",
                    textures.len(),
                    texture_dir
                );
            }
    ...
    pub fn get_texture(&self, block_name: &str, face: &str) -> Option<String> {
        self.textures
            .get(block_name)
            .and_then(|texture| match face {
                "top" => texture.top.clone().or_else(|| Some(texture.main.clone())),
                "bottom" => texture
                    .bottom
                    .clone()
                    .or_else(|| Some(texture.main.clone())),
...
mod tests {
    ...
    fn test_extract_base_name() {
        ...
        assert_eq!(
            extract_base_name("stone_bricks_side"),
            ("stone_bricks", Some("side"))
        );
        assert_eq!(
            extract_base_name("stone_bricks_bottom"),
            ("stone_bricks", Some("bottom"))
        );
    }
}
