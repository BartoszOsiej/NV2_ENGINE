...
impl BlockType {
    ...
    pub fn break_time_seconds(self) -> Option<f32> {
        match self {
            ...
            BlockType::Sand | BlockType::Gravel | BlockType::Snow => {
                Some(self.hardness() as f32 * 0.08)
            }
            BlockType::Grass
            | BlockType::Dirt
            | BlockType::Clay
    ...
    pub fn has_model(self) -> bool {
        ...
        !texture_name.is_empty()
            && crate::assets::BlockModelLoader::get_model(texture_name).is_some()
    }

    pub fn has_registered_texture(self) -> bool {
        let texture_name = self.texture_name();
        !texture_name.is_empty()
            && crate::renderer::texture_atlas::tile_by_texture_name(texture_name, false).is_some()
    }
...
mod tests {
    ...
    fn canopy_blocks_expose_foliage_medium() {
        for block in [
            ...
            let medium = block
                .movement_medium()
                .expect("expected foliage movement medium");
    ...
    fn mineral_blocks_report_expected_tool_tiers() {
        ...
        assert_eq!(
            BlockType::IronOre.required_tool_tier(),
            Some(ToolTier::Stone)
        );
        assert_eq!(
            BlockType::DiamondOre.required_tool_tier(),
            Some(ToolTier::Iron)
        );
        assert_eq!(
            BlockType::Obsidian.required_tool_tier(),
    ...
    fn log_blocks_require_iron_tier_hardness_and_leaves_stay_soft() {
        for block in [
            ...
            assert_eq!(
                block.hardness(),
                5,
                "{} should require iron-tier power",
                block.name()
            );
        }
    ...
    fn registered_world_blocks_do_not_accidentally_default_to_zero_hardness() {
        for (id, name, _) in BLOCK_REGISTRY {
            ...
            if !zero_hardness_is_intended {
                assert!(
                    block.hardness() > 0,
                    "{} unexpectedly has zero hardness",
                    name
                );
            }
        }
